...
function createCarouselIndicatorsHTML(images) {
    ...
    images.forEach((img, idx) => {
        html +=
            `<button type="button" data-bs-target="#carouselExampleAutoplaying" data-bs-slide-to="${idx}" class="${idx === 0 ? 'active' : ''}" ${idx === 0 ? 'aria-current="true"' : ''} aria-label="Slide ${idx + 1}"></button>`
    })
    return html
}
...
function createWidgetHTML(widget) {
    ...
    items.forEach(item => {
        if (widget.type === 'color') {
            ...
            itemHTML +=`<div class="col-12">
            <div class="border border-primary border-3 rounded-3 p-4" role="button" onclick="clickHandler(this,'${widget.type}')">${item}</div>
        </div>`
        }
    })
